  ...
  render() {
    ...
    let titleViews = titles.map((title, index) => {
        ...
        const titleView = this.props.renderTitle ? this.props.renderTitle(index, title, isSelected) : ( <
          Text style = {
            isSelected ? [styles.titleTextSelected, selectedItemTextStyle] : [styles.titleText, itemTextStyle]
          } > {
            title
          } <
          /Text>
        ...
        return( <
          TouchableOpacity style = {
            [styles.titleContainer, itemMarginObj, isSelected ? selectedItemStyle : itemStyle]
          }
          activeOpacity = {
            0.6
          }
          ...
          onPress = {
            ...
            isSelected ? < View style = {
              [styles.selectedBorder, selectedBorderStyle]
            }
            /> : null} < /
            TouchableOpacity >
          )
        ...
        View style = {
          [styles.indicatorContainer, this.props.style]
        } >
        <
    ...
    _visibleDetect(selectedIndex) {
      ...
      if(moveDir === FORWARD) {
        ...
        if(this._contentHorOffset > lastItemOffsetX) {
          ...
          this.scroller.scrollTo({
            x: this._contentHorOffset + deltaX
          });
        }
